...
export interface ExperienceContextPack {
  // Universal layer — present in every experience
  universal: {
    language: string                    // "en-GB", "en-US", etc.
    outputFormat: string                // "second-person-present-tense-prose"
    targetNodeLength: {
      min: number                       // words
      max: number                       // words
    }
    contentRating: "general" | "teen" | "mature" | "explicit"
    tone: string                        // prose description of overall tone
    forbiddenContent: string[]          // explicit list of forbidden elements
    styleNotes: string                  // prose style guidance
  }
  
  // Use case layer — varies by experience type
  useCaseContext: StoryWorldContext | CompanyContext | SubjectContext | IPContext
}
...
export interface StoryWorldContext {
  ...
  setting: {
    timePeriod: string
    location: string
    physicalRules: string             // what kind of world is this
  }
  atmosphere: {
    genre: string
    emotionalRegister: string
    prosStyleReference: string
    pacingNotes: string
  }
  ...
  canonicalFacts: string[]           // things that are true and immovable
  forbiddenElements: string[]        // specific to this story world
  motifs: string[]                   // recurring images, phrases, details
}
...
export interface CompanyContext {
  ...
  policyDocumentIds: string[]        // Supabase Storage file IDs
  characterPersonas: CharacterDefinition[]
  assessmentCriteria: {
    passThreshold: number
    feedbackTone: "coaching" | "formal" | "encouraging"
  }
}
...
export interface SubjectContext {
  ...
  pedagogicalApproach: "socratic" | "case_study" | "simulation"
...
### Node Definitions (Phase 1 — 5 types)
...
export interface FixedNode extends BaseNode {
  ...
  nextNodeId: string                 // always has exactly one next node
}
...
export interface GeneratedNode extends BaseNode {
  ...
  // What the AI should produce at this beat
  beatInstruction: string            // what dramatic/emotional state to achieve
  
  // Generation constraints
  constraints: {
    lengthMin: number                // words
    lengthMax: number                // words
    mustEndAt: string                // "natural pause before choice"
    mustNotDo: string[]              // specific prohibitions for this node
    mustInclude?: string[]           // optional required elements
  }
  
  // The AI will also receive automatically:
  // - full Experience Context Pack
  // - full narrative history from session
  // - current session state
  // - arc awareness (depth, distance to endpoint, pacing instruction)
...
export interface ChoiceNode extends BaseNode {
  ...
  // For open (free text) choices
  openPrompt?: string                // the question posed to the reader
  openPlaceholder?: string           // input placeholder text
}

export interface ChoiceOption {
  id: string
  label: string                      // the text shown to the reader
  nextNodeId: string
  isLoadBearing: boolean
  
  // Depth gate — suppress option until minimum choices made
  depthGate?: {
...
export interface CheckpointNode extends BaseNode {
  type: "CHECKPOINT"
  visible: boolean                   // show confirmation to reader or silent
  visibleContent?: string            // if visible, what to show
  marksCompletionOf: string          // section label
  unlocks: string[]                  // node ids or feature flags unlocked
  nextNodeId: string
}
...
export interface EndpointNode extends BaseNode {
  type: "ENDPOINT"
  endpointId: string                 // matches an EndpointShape id
  outcomeLabel: string               // "Discovery", "Escape", "Lost"
  
  // Fixed closing line — the line the reader remembers and shares
  // Written by the author, never generated
  closingLine: string
  
  // AI generates the personalised journey summary
  // (draws from full narrative history and session state)
  summaryInstruction: string         // what the summary should reflect on
  
  // Outcome card configuration
  outcomeCard: {
    shareable: boolean
    showChoiceStats: boolean         // "67% of readers chose the same path"
    showDepthStats: boolean          // "You explored 34% of this story"
...
export interface SessionState {
  // Accumulated from state_changes across all choices
  flags: Record<string, number | string | boolean>
  
  // Path tracking
  currentPath: string               // e.g. "contact" | "escape" | "crew"
  choicesMade: number
  nodesVisited: string[]
  
  // Arc awareness (calculated by engine, not stored by author)
  depthPercentage: number           // 0-100
  distanceToNearestEndpoint: number // estimated choices remaining
  pacingInstruction: string         // injected into generation prompts
  
  // Performance tracking
  generationTimings: Record<string, number> // nodeId -> ms
}
```
---
