...
type PageStatus =
  ...
  | { type: "advancing" } // fetching CHOICE node after prose
...
export default function StoryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  function handleNodeContent(
    ...
    if (content.type === "prose") {
      ...
      // Show generating screen only for the opening scene
      ...
      // Silent checkpoint — auto-advance
  ...
  if (status.type === "generating") {
    return (
      <GeneratingScreen
        ...
        onReady={() =>
          advanceToNextNode(status.sessionId, 0)
        }
      />
    )
  }
