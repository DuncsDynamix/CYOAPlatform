...
//
// Stashed on globalThis (same trick as lib/db/prisma.ts) because Next.js
// dev (Turbopack) compiles each API route file as its own module graph —
// a plain module-level `const` gets a fresh, empty Map per route, so
// pre-generation done under POST /start would be invisible to a later
// GET /node or POST /choose request even within the same process. Without
// this, every pre-generated node is silently regenerated the first time a
// *different* route reads it back.

const MAX_MEM_ENTRIES = 500
const globalForCache = globalThis as unknown as {
  __traverseMemCache: Map<string, { content: string; expiresAt: number }> | undefined
}
const memCache = globalForCache.__traverseMemCache ?? new Map<string, { content: string; expiresAt: number }>()
if (process.env.NODE_ENV !== "production") globalForCache.__traverseMemCache = memCache
